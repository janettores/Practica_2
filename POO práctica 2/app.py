...
def listarEstudiantesAsistencia():
	...
	if  request.method == 'POST':
		...
		estudiantes = Estudiante.query.filter_by(idcurso = 2).all() #Estudiante.query.filter(idcurso = 2).order_by(Estudiante.apellido, Estudiante.nombre).all()
...
def informeDetalles():
	...
	if request.method == 'GET':
		if session['rol'] is not None and  session['rol'] == 'preceptor' :
			idpreceptor = session['id']
			preceptor = Preceptor.query.filter_by(id=idpreceptor).first()
			return  render_template('informeDetalles.html', cursos = preceptor.cursos)
	elif request.method == 'POST':
		curso = request.form['curso']
		estudiantes = Estudiante.query.filter_by(idcurso = curso)
		aux = []
		for estudiante in estudiantes:
			...
			total_inasistencias = asistencias_aula_ausente_justificadas + asistencias_aula_ausente_injustificadas + asistencias_educacion_fisica_ausente_justificadas + asistencias_educacion_fisica_ausente_injustificadas
		...
		return render_template('informeDetalles.html', estudiantes = estudiantes.all(), cantidades = aux, informar = True )
